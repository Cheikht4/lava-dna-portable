...
| **Modules Perl (lib/)** | 14 fichiers | 21 fichiers (+7 nouveaux) | +7 modules |
...
| **Script utilitaire (`slava.pl`)** | 1 (9808 octets) | 0 (non supporté actuellement) | -1 fichier |
...
### 1.2 `slava.pl` → Retiré (Non supporté dans la V3)
- **Rôle original** : Script "Sliding LAVA". Ce wrapper découpait les très longues séquences génomiques (ex: génomes entiers de 30kb) en segments chevauchants (`slava_segment_max_length`) pour paralléliser ou accélérer l'analyse de Primer3.
- **Raison** : N'a pas été porté dans la nouvelle architecture Python/Flask. Actuellement, l'interface web soumet le génome entier d'un coup. (C'est une fonctionnalité qui pourrait être réintégrée dans le backend Python à l'avenir si l'analyse de génomes complets devient trop lente).
...
| Fonction de pénalité géométrique | Unifiée via `PipelineUtils.pm` | Unifiée via `PipelineUtils.pm` |
| Résolution chevauchement | `max_overlap_percent`, `resolve_overlap_by` | `max_overlap_percent`, `resolve_overlap_by` (penalty/coverage) |
| Validation signature | Unifiée via `PipelineUtils.pm` | Unifiée via `PipelineUtils.pm` |
| Tags stockés dans signature | Unifiés via `PipelineUtils.pm` | Unifiés via `PipelineUtils.pm` |
...
### 3.5 `lib/LLNL/LAVA/PipelineUtils.pm` [NOUVEAU] — 983 lignes
**Rôle** : Module de centralisation des pipelines STEM et LOOP (Phase 36 d'harmonisation)

**Fonctions exportées** :
- **`calculateSignatureIntersection()`** — Logique unifiée d'intersection de signatures (6 ou 8 amorces).
- **`createPerSignatureFiles()`** — Export unifié des résultats par signature avec statuts et détails IUPAC.
- **`createAmplificationFiles()`** — Export unifié des fichiers FASTA d'amplification.
- **`calculateDynamicPairLengths()`** — Calcul partagé et dynamique des distances inter-amorces cibles.
- **`reduceSignaturesByOverlap()`** — Harmonisation de la résolution des chevauchements.
**Impact** : A permis de supprimer plus de 1000 lignes de code dupliqué entre les scripts STEM et LOOP, garantissant une stricte parité algorithmique entre les deux approches.
...
| Export des résultats | Fichier texte brut | Rapports par signature + FASTA + Fichiers `.params.txt` de traçabilité |
...
*Document mis à jour le 26/04/2026 — Analyse bioinformatique comparative LAVA-DNA (Incluant la Phase 36 d'harmonisation)*
