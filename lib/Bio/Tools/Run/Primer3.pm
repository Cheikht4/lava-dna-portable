...
BEGIN { 
 ...
 SEQUENCE_TEMPLATE SEQUENCE_ID SEQUENCE_INTERNAL_OLIGO
 PRIMER_TASK PRIMER_PICK_INTERNAL_OLIGO PRIMER_PICK_ANYWAY
 PRIMER_DNTP_CONC PRIMER_SALT_CONC PRIMER_DNA_CONC PRIMER_THERMODYNAMIC_PARAMETERS_PATH
 PRIMER_MAX_TEMPLATE_MISPRIMING PRIMER_PAIR_MAX_TEMPLATE_MISPRIMING PRIMER_TM_FORMULA PRIMER_SALT_CORRECTIONS
 PRIMER_INTERNAL_SALT_MONOVALENT PRIMER_INTERNAL_SALT_DIVALENT PRIMER_INTERNAL_DNTP_CONC PRIMER_INTERNAL_DNA_CONC
 PRIMER_INTERNAL_MIN_TM PRIMER_INTERNAL_MAX_TM PRIMER_INTERNAL_OPT_TM
...
sub new {
	...
	if ($args{'-seq'}) {
		...
		push (@input, ("PRIMER_SEQUENCE_ID=".$self->{'seqobject'}->id),
				("SEQUENCE_TEMPLATE=".$self->{'seqobject'}->seq));
	...
	foreach my $key (keys %args) {
		...
		if ((uc($key) eq "-SEQ") || (uc($key) eq "-SEQUENCE")) {
			...
			push (@$inputarray, ("PRIMER_SEQUENCE_ID=".
			  $self->{'seqobject'}->id),("SEQUENCE_TEMPLATE=".$self->{'seqobject'}->seq));
			next;
		}
		...
		if (uc($key) eq "INCLUDED_REGION") {
			...
			foreach my $input (@$inputarray) {
				if ($input =~ /SEQUENCE_TEMPLATE=(.*)/) {$sequencelength=length($1)}
			}
		...
		elsif (uc($key) eq "SEQUENCE_TEMPLATE") {
...
sub run {
  ...
  if ( $executable =~ m{^[^\'\"]+(.+)[^\'\"]+$} ) { 
	...
	$self->{results_obj}->_set_variable('seqobject', $self->{seqobject});
 	...
 	my %input_hash = map { split('=', $_, 2) } @{$self->{'primer3_input'}};
...
sub _input_args {
  ...
  'SEQUENCE_TEMPLATE'=>'(nucleotide sequence, REQUIRED) The sequence itself. Cannot contain newlines',
