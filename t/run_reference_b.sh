...
echo "Running Test B (Loop)..."
perl lava_loop_primer.pl \
  --alignment_fasta t/fixtures/single_400.fasta \
  --threads 1 \
  --signature_max_length 400 \
  --window_size 20 \
  --max_per_window 3 \
  --output_file t/reference_b_loop \
  --loop_min_gap 20 \
  ...
  --max_overlap_percent 0.0 --max_primer_gen 10000.0 > t/reference_b_loop.log 2>&1

echo "Test B (Loop) Done."
