...
"""
  ...
  my $minS_innerToStem_F = @$innerToStemPenalties_r ? $min_val_f->(@$innerToStemPenalties_r) * $innerToStemPenaltyWeight : 0;
  ...
  my $rmq_stem_f   = build_rmq($masterStemF_data_r, 2) if $includeStemPrimers;
    ...
    bb_mid_out_fwd = """
              ...
              if ($m_start != -1 && $m_end != -1 && $m_start <= $m_end) {
                  ...
                  my $base_penalty = ($innerPenalty * $innerPenaltyWeight) + 
                                     ...
                                     ($includeStemPrimers ? $innerToStemPenalties_r->[$innerToStemDistance] * $innerToStemPenaltyWeight : 0);
                  my $min_S_to_mid = $includeStemPrimers ? $minS_stemToMiddle_F : 0; 
                  # in stem script it's just innerToMiddlePenalties_r
                  
                  if ($base_penalty + $min_P_mid_range + $min_P_outer_F + $minS_stemToMiddle_F + $minS_middleToOuter_F >= $bestSetPenalty) {
                          ...
                          if ($o_start != -1 && $o_end != -1 && $o_start <= $o_end) {
                              ...
                              if ($part_penalty + $min_P_out_range + $minS_middleToOuter_F >= $bestSetPenalty) {
                                  ...
                                  for(my $k = $o_start; $k <= $o_end; $k++)
                                  {
                                      ...
                                      if ($includeStemPrimers) {
                                          $spacingPenalty = ($innerToStemPenalties_r->[$innerToStemDistance] * $innerToStemPenaltyWeight) +
                                                            ($innerToMiddlePenalties_r->[$innerToMiddleDistance] * $innerToMiddlePenaltyWeight) +
                                          ...
                                          $detailStr = sprintf("Spc[I_S:%.1f I_M:%.1f M_O:%.1f] Thm[I:%.1f S:%.1f M:%.1f O:%.1f]", 
                                                ($innerToStemPenalties_r->[$innerToStemDistance] * $innerToStemPenaltyWeight),
                                                ($innerToMiddlePenalties_r->[$innerToMiddleDistance] * $innerToMiddlePenaltyWeight),
                                                ($middleToOuterPenalties_r->[$middleToOuterDistance] * $middleToOuterPenaltyWeight),
...
"""
    ...
    finish_fwd_block_repl = r"""\1          
          ...
          if ($chunk_done % 5 == 0 || $chunk_done == int(($innerForwardCount - $chunk_id + $num_fwd_chunks - 1) / $num_fwd_chunks)) {
              if (open my $fh, '+<', $fwd_prog_file) {
                  flock($fh, 2);
                  my @lines = <$fh>;
                  $lines[$chunk_id] = "$chunk_done,$chunk_hits,$chunk_pruned,$chunk_evaluated\n";
                  seek($fh, 0, 0);
                  print $fh @lines;
                  truncate($fh, tell($fh));
...
"""
  ...
  my $minS_innerToStem_R = @$innerToStemPenalties_r ? $min_val_f->(@$innerToStemPenalties_r) * $innerToStemPenaltyWeight : 0;
  ...
  my $rmq_stem_r   = build_rmq($masterStemR_data_r, 2) if $includeStemPrimers;
  ...
  \\1"""
    ...
    bb_mid_out_rev = """
              ...
              if ($m_start != -1 && $m_end != -1 && $m_start <= $m_end) {
                  ...
                  if ($base_penalty + $min_P_mid_range + $min_P_outer_R + $minS_stemToMiddle_R + $minS_middleToOuter_R >= $bestSetPenalty) {
                      $chunk_pruned += ($m_end - $m_start + 1);
                  } else {
                      for(my $j = $m_start; $j <= $m_end; $j++)
                      {
...
"""
    ...
    finish_rev_block_repl = r"""\1          
          # Intra-chunk progress reporting
          if ($chunk_done % 5 == 0 || $chunk_done == int(($innerReverseCount - $chunk_id + $num_rev_chunks - 1) / $num_rev_chunks)) {
              if (open my $fh, '+<', $rev_prog_file) {
                  flock($fh, 2);
                  my @lines = <$fh>;
                  $lines[$chunk_id] = "$chunk_done,$chunk_hits,$chunk_pruned,$chunk_evaluated\n";
