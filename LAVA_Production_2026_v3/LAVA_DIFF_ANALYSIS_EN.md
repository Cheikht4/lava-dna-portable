...
| **Perl modules (lib/)** | 14 files | 20 files (+6 new) | +6 modules |
...
| **Utility script (`slava.pl`)** | 1 (9808 bytes) | 0 (removed) | -1 file |
...
### 1.2 `slava.pl` → Removed
- **Original role**: Auxiliary LAVA launcher script (9808 bytes)
- **Reason**: Functionality absorbed by the new Flask web interface and the `launch_lava_smart_kill.py` script
...
| Geometric penalty function | `calculate_proportional_geometry` | `generateSigmoidPenalty` |
| Overlap resolution | No | `max_overlap_percent`, `resolve_overlap_by` (penalty/coverage) |
| Signature validation | Returns 2 values | Returns 3 values (+ `validation_status`) |
| Tags stored in signature | Basic coverage | `signature_intersection_ids`, `signature_coverage_percent`, `signature_target_count`, `validation_status`, `primer_coverage_details` |
...
**Operation**: Converts named parameters `stem_primer_*` to internal Primer3 tags (`PRIMER_INTERNAL_*`), enabling a clear user interface.
...
### 9.1 Genomic Diversity Management
...
| Utility modules | 0 | 4 new (`Core.pm`, `Validator.pm`, `Lava::Core`, `StemConserved.pm`) |
| User interface | CLI only | CLI + Flask Web Interface |
| Deployment | Manual | Automated (Nginx + Gunicorn + systemd) |
| Results export | Plain text file | Per-signature reports + amplified/excluded FASTA |
| Post-design validation | None | Coverage intersection + combinatorial analysis |
...
*Document generated on 04/24/2026 — Comparative bioinformatics analysis of LAVA-DNA*
