################################################################################
#
# LLNL::LAVA::PipelineUtils - Fonctions utilitaires partagées du pipeline LAVA
...
# et lava_loop_primer.pl, éliminant la duplication de code.
...
sub reducePairInfosByPenalty
{
  ...
  # Sort the pairs by penalty / Tri par pénalité
  my @sortedInfos =
    map { $_->[0] }
    sort { $a->[1] <=> $b->[1] }
    map {[$_, $_->getPenalty()] }
...
sub reduceSignaturesByOverlap
{
  ...
  # Sort by resolve strategy / Tri selon la stratégie de résolution
