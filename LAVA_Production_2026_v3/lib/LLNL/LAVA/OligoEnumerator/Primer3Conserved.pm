...
sub new
{
  ...
  my $p3Names_r = {
    ...
    # Paramètres SantaLucia 1998 et Owczarzy 2004 pour LAMP isotherme à 65°C
  ...
  my $p3Targets_r = {
    ...
    # Optimisé pour amplification LAMP isotherme à 65°C
  ...
  # Paramètres LAVA personnalisés (non-Primer3)
  my $lavaParams_r = {
...
# }
  ...
  for (my $pos = 1; $pos <= $alignmentLength; $pos++) {
      ...
      foreach my $seq ($alignment->each_seq()) {
          ...
          # Mais même 'N' pourrait être compté comme un état... restons simple : N ignoré, Gap compté.
          if ($base eq '-') {
              $baseComments{'-'}++;
              $gapCount++;
              $count++;
      ...
      # Règle de pénalité GAPS : Si > 20% de gaps, zone invalide (entropie forcée au max)
  ...
  # Identifier les zones à haute entropie
  ...
  my $inHighEntropy = 0;
  my $startHigh = 0;
  # Récupérer le seuil d'entropie depuis les paramètres LAVA (défaut 1.5)
  my $lavaParams_r = $this->{"d_lavaParams"};
  my $entropyThreshold = $lavaParams_r->{"entropy_threshold"} || 1.5;
  
  # Récupérer la taille de fenêtre (taille MINIMALE de l'amorce pour être permissif)
  ...
  for (my $i = 0; $i < scalar(@entropies) - $windowSize + 1; $i++) {
      ...
      # Si la moyenne sur la fenêtre dépasse le seuil, on marque le CENTRE de la fenêtre comme à exclure
      # Ou plus simplement, si une fenêtre est mauvaise, on ne devrait pas pouvoir commencer un primer ici
      # Mais Primer3 attend des régions exclues, pas des points de départ interdits.
      
      # Approche : Si moyenne > seuil, toute la fenêtre est considérée comme "à risque"
  ...
  if ($oligoCount == 0) {
      # DEBUG LAVA 2026 : Diagnostiquer pourquoi 0 résultats
      print "⚠️  AUCUN OLIGO TROUVÉ. Diagnostic :\n";
      
      # Afficher les régions exclues
      if (@excludedRegions) {
      ...
      # BioPerl ne donne pas toujours accès facile au EXPLAIN complet, mais on essaie
      # On va regarder les résultats bruts s'ils sont accessibles
      eval {
  ...
  # Les oligos sont retournés directement (seq originale).

  # FILTRE HOMOPOLYMÈRES (Post-Processing LAVA 2026)
  # Primer3 ne semble pas toujours respecter PRIMER_INTERNAL_OLIGO_MAX_POLY_X 
  # ou il y a un conflit de paramètres. On filtre manuellement.
  
  my $maxPolyBases = $p3Targets_r->{"PRIMER_INTERNAL_OLIGO_MAX_POLY_X"};
  # Si non défini dans p3Targets, voir si défini dans lavaParams ou défaut
  if (!defined $maxPolyBases) {
     $maxPolyBases = $lavaParams_r->{"max_poly_bases"} || 5; # Défaut permissif si non défini
  }
...
# Fin de la classe sans écraser les méthodes suivantes si elles existent (mais setPrimer3Targets est après)
...
sub setPrimer3Targets
{
  ...
  {
    # Gérer les paramètres LAVA personnalisés séparément
