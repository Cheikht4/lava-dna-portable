...
### Date/Étape : 2026-07-25 - Renommage UI des amorces internes (F1C/B1C vers F1/B1)
...
**Explication technique :** 
Remplacement des chaînes de caractères "F1c", "B1c", "F1C" et "B1C" par "F1" et "B1" dans tous les messages imprimés émis vers STDOUT, notamment ceux passés à la fonction `checkPrimerMismatchTolerance`. Les clés internes des structures de données (`%isFixedType{"F1C"}`, `@all_types`) ont été conservées intactes pour garantir la rétrocompatibilité des fichiers de paramètres existants.

**Justification biologique :** 
Bien que, du point de vue thermodynamique, les amorces internes se lient au brin complémentaire (d'où l'appellation F1c/B1c signifiant "Complement"), la convention standard en biologie moléculaire pour le design LAMP désigne les paires par F1/B1. Exposer la nomenclature "c" sur l'interface (via la barre de progression) générait une confusion cognitive chez les chercheurs qui pouvaient interpréter "F1c" comme un type d'amorce distinct.
...
### Date/Étape : 2026-08-02 / Chantier 6 (Batching et contrôle mémoire)
**Fichiers impactés** : `lava_loop_primer.pl`, `lava_stem_primer.pl`
**Nature du changement** : Architecture / Mémoire
**Explication technique** : Modification de la boucle de création des signatures (combinaison des paires Forward/Reverse). Au lieu de matérialiser l'intégralité des millions de signatures LAMP dans un tableau global `@allFoundSignatures` avant de lancer la validation, le processus traite désormais les signatures par *lots* (batches). Chaque lot de taille `--assembly_batch_size` (défaut: 50000) est envoyé au `ForkManager` pour validation parallèle dès qu'il est rempli. Seules les signatures valides sont conservées dans `@retained_signatures`. Un plafond d'éviction `--max_retained_signatures` (défaut: 10000) limite la taille de ce tableau : s'il déborde, les signatures sont triées par pénalité thermodynamique et les moins bonnes sont évincées.
**Justification biologique** : L'abolition des plafonds géométriques stricts permettait d'explorer un espace de design d'amorces très vaste, crucial pour les virus très variables, mais provoquait une explosion mémoire du moteur Perl (plusieurs Go). En traitant les candidats par lots et en ne gardant que le *Top N*, la mémoire du processus reste faible et stable (de l'ordre d'une centaine de Mo), tout en conservant la capacité d'évaluer plusieurs millions de combinaisons potentielles sans perte d'efficacité.
**Impact attendu** : Fin des plantages liés au manque de mémoire RAM (`Out of Memory`). Le moteur peut désormais tourner sur de très petits ordinateurs portables, même sur des jeux de données denses (ex: Dengue). L'utilisateur verra un avertissement si des candidats valides ont dû être évincés, et le fichier d'archive `.all_signatures` ne contiendra plus que les signatures retenues au lieu de millions de lignes inexploitables.

### Date/Étape : 2026-08-02 / Optimisation de l'intersection (Chantier 6 - Perf)
**Fichiers impactés** : `lib/LLNL/LAVA/PipelineUtils.pm`
**Nature du changement** : Algorithmique / Performance
**Explication technique** : Suppression d'un tri coûteux (`sort keys %intersection_set`) situé à l'intérieur de la boucle la plus chaude de la fonction `calculateSignatureIntersection`. Ce tri avait été introduit lors de la résolution du problème de non-déterminisme, mais s'avérait inutile à ce stade intermédiaire (l'intersection d'ensembles est commutative/associative). La liste finale d'identifiants n'est désormais triée qu'une seule fois à la fin de la fonction (`@final_ids = sort keys %intersection_set;`).
**Justification biologique** : La validation des signatures (vérification thermodynamique et calcul de la couverture) générait une charge processeur exponentielle. Pour chaque signature composée de 6 ou 8 amorces, l'ensemble des cibles virales potentielles (parfois des centaines, comme pour les grands clusters de Dengue) était itéré et re-trié à chaque étape d'intersection, transformant l'algorithme d'inclusion en un goulot d'étranglement majeur.
**Impact attendu** : Accélération spectaculaire de la phase "Combinaison & Validation" (multipliée par plusieurs centaines). L'algorithme préserve un déterminisme parfait (l'ordre des séquences en sortie reste identique) mais le coût de calcul devient quasi-nul, permettant d'effectuer la validation croisée en quelques minutes au lieu de dizaines d'heures.

### Date/Étape : 2026-08-02 / Optimisation de l'intersection par vecteurs de bits (Chantier 6 - Perf)
**Fichiers impactés** : `lib/LLNL/LAVA/PipelineUtils.pm`
**Nature du changement** : Algorithmique / Performance
**Explication technique** : Remplacement des tables de hachage par des vecteurs de bits (bitstrings) pour l'opération `calculateSignatureIntersection`. Au lieu de recréer 8 tables de hachage pour chaque signature évaluée (soit des millions d'allocations redondantes), la couverture de chaque amorce est sérialisée une seule fois lors de son instanciation (`vec($vec, $id, 1) = 1`) via un nouveau tag `compatible_sequence_vec`. L'intersection devient alors une unique opération binaire hyper-rapide (`$a & $b`).
**Justification biologique** : Les virus avec une grande diversité génomique (comme Dengue) génèrent de larges alignements et nécessitent des assemblages multi-amorces dont la composante majeure du temps de calcul réside dans le chevauchement (ou intersection) des couvertures de chaque composant (F3, B3, F2, etc.). Le hachage dynamique asphyxiait le calcul, là où l'opérateur AND binaire traite la séquence par blocs de 32 bits en O(1) machine.
**Impact attendu** : Une chute drastique du temps de calcul de la phase "Combinaison & Validation" (gain d'au moins un ordre de grandeur) permettant de retrouver une fréquence d'évaluation comparable aux étapes unitaires Forward/Reverse, sans la moindre augmentation de l'empreinte RAM (le vecteur de bits occupant au maximum quelques octets par amorce). L'équivalence est totale et déterministe.

### Date/Étape : 2026-08-02 / Contournement de la reconstruction du tableau d'identifiants (Chantier 6 - Perf 2)
**Fichiers impactés** : `lib/LLNL/LAVA/PipelineUtils.pm`, `lava_loop_primer.pl`, `lava_stem_primer.pl`
**Nature du changement** : Algorithmique / Performance
**Explication technique** : La précédente optimisation par vecteurs de bits était bridée par la Phase 3 de l'intersection, qui reconstruisait naïvement et systématiquement un tableau complet (`@final_ids`) de séquences compatibles pour des millions de combinaisons, alors même que le processus n'en conservait que la taille (le décompte `scalar(@final_ids)`). La reconstruction a été conditionnée par l'ajout du paramètre optionnel `$return_list` (à 1 par défaut pour ne pas briser les anciens scripts non patchés). Les boucles chaudes du processus principal invoquent désormais la méthode avec la valeur 0, forçant un retour immédiat en une seule instruction machine `unpack("%32b*")`, et réservent la reconstruction intégrale du tableau (`$return_list = 1`) uniquement aux quelques dizaines de signatures survivantes.
**Justification biologique** : Lors du criblage bio-informatique in-silico, la grande majorité des assemblages virtuels est rejetée pour non-respect de la couverture minimale des variants. Reconstruire la topographie précise des génomes ciblés pour une amorce condamnée est une pure perte thermodynamique. 
**Impact attendu** : Une élimination totale du temps exponentiel du goulot d'étranglement de `Combinaison & Validation`, la propulsant d'une vitesse de l'ordre du singulier à une vitesse par lots (centaines de milliers d'itérations par seconde). L'équivalence logique reste pure et parfaite.

### Date/Étape : 2026-08-02 / Refonte Map-Reduce de la Combinaison (Chantier 6 - Stabilité Mémoire/Multiprocessing)
**Fichiers impactés** : `lava_loop_primer.pl`, `lava_stem_primer.pl`
**Nature du changement** : Architecture / Mémoire
**Explication technique** : Remplacement du gestionnaire de batchs `Parallel::ForkManager` basé sur un `fork` itératif à chaque lot par une architecture partitionnée en amont de type "Map-Reduce". Au lieu de regrouper les combinaisons et de multiplier les processus, le script principal instancie une seule fois les `threads` au début de la phase. Chaque *worker* se voit attribuer un identifiant unique (chunk_id) et évalue exclusivement une partition statique des données d'entrée (sauts d'indices de type `i += num_chunks`). Les *workers* maintiennent et réduisent localement leur propre liste des meilleures amorces, et ne renvoient au processus parent qu'un tableau de scalaires (indices originaux et couverture) plutôt qu'un arbre d'objets Perl lourds. Le processus parent effectue l'assemblage final des objets `LAMP` uniquement sur les 10 000 meilleures combinaisons. Le déterminisme absolu a été garanti en triant par `chunk_id` la reconstruction finale de la liste dans le parent avant de trier par `lamp_penalty`.
**Justification biologique** : Lors de la combinaison de millions d'amorces, la sérialisation inter-processus via `Storable` d'objets orientés-objets complexes induit une explosion exponentielle de l'empreinte mémoire et une dégradation des performances à cause des accès "Copy-on-Write" du kernel. Séparer l'évaluation cinétique d'hybridation (le calcul pur) de la reconstruction des objets (l'assemblage final de la signature candidate) garantit la stabilité thermodynamique des calculs à l'échelle génomique.
**Impact attendu** : Élimination absolue et définitive du goulot mémoire en mode parallèle (`--threads > 1`). La vitesse de traitement sur de très grandes cohortes de variants (ex: virus Dengue très variables) augmente radicalement, tout en restant parfaitement économe en RAM.

### Date/Étape : 2026-08-03 / Branchement de la pénalité d'empan interne F1c-B1c (Chantier 7 - Thermodynamique LOOP)
**Fichiers impactés** : `lava_loop_primer.pl`
**Nature du changement** : Thermodynamique / Bug Fix
**Explication technique** : La distance F1c-B1c (l'empan interne de la signature) était historiquement contrainte par une limite stricte `next if ($inner_gap > 100)`. Depuis la suppression de cette limite stricte au profit du modèle proportionnel, la pénalité censée évaluer la distance n'était pas branchée dans l'étape de création de la signature. Bien que la table `$innerToInnerPenalties_r` fût calculée (avec une cible à 40% de la longueur totale) et qu'un coefficient `$innerForwardToReversePenaltyWeight` fût défini, ces valeurs étaient ignorées, laissant l'empan F1c-B1c sans la moindre pénalisation géométrique. 
...
### Date/Étape : 2026-08-04 / Recalibrage géométrique et Pénalité de longueur totale (Chantier 8 - Thermodynamique Avancée)
**Fichiers impactés** : `lib/LLNL/LAVA/Core.pm`, `lava_loop_primer.pl`, `lava_stem_primer.pl`
**Nature du changement** : Thermodynamique / Architecture
**Explication technique** : 
1. La géométrie des espacements (`calculate_proportional_geometry`) ne définit plus des cibles ponctuelles rigides (ex: 12% de `total_signature_length`). Elle prend désormais en compte l'espace réel disponible en soustrayant la somme des longueurs des amorces (`$sum_primer_lengths`) de la longueur totale visée. Elle retourne des "fourchettes" d'espacement de type `[cible_optimale, borne_haute]` pour chaque segment.
2. Les pénalités d'espacement (générées via `generateDistancePenalties` dans `Core.pm`) utilisent désormais ces `borne_haute` (qui intègrent le plateau de tolérance gratuit conservé à 0.25) comme point de départ de la sigmoïde pénalisante.
3. Implémentation d'une nouvelle option globale `--signature_length_penalty_weight` (par défaut 10). Dans la phase finale d'assemblage (`Combining Best F/R Halves`), la longueur totale de l'amplicon (`$totalLen_v`) est comparée à `--total_signature_length`. Toute signature plus courte n'est pas pénalisée. Toute signature plus longue subit une pénalité sigmoïde additionnelle plafonnée par ce poids, qui est injectée directement dans le score final `lamp_penalty` et affichée dans le diagnostic (`Spc[Len:%.1f]`).
**Justification biologique** : Les cibles d'espacement dérivaient précédemment toutes de la `total_signature_length` brute, sans tenir compte de la longueur occupée physiquement par les amorces. Une demande d'allongement de la signature entraînait donc un déplacement chaotique de l'optimalité de tous les espacements en même temps, forçant la sélection de jeux d'amorces sous-optimaux. En raisonnant sur l'espace internucléotidique réel disponible, l'algorithme permet d'ajuster la longueur cible globale sans détruire l'équilibre des espacements inter-amorces. La pénalité asymétrique sur la longueur totale favorise strictement des amplicons compacts (cinétique rapide) tout en tolérant un étirement minime via une pente sigmoïde douce.
**Impact attendu** : Une séparation propre entre le contrôle des espacements locaux (qui restent proportionnels à l'espace libre) et le contrôle de l'encombrement global. La manipulation de `total_signature_length` ne sacrifie plus la qualité des amorces (Tm, ΔG) au profit de l'espacement.
...
### Date/Étape : 2026-08-06 - Optimisation anti-saturation des logs
**Fichiers impactés** : `lava_loop_primer.pl`, `lava_stem_primer.pl`, `lib/LLNL/LAVA/PipelineUtils.pm`
**Nature du changement** : Architecture / Bug Fix
**Explication technique** : Suppression des instructions `print` inconditionnelles situées au cœur des boucles de combinaisons et de vérification spatiale (ex: `inner_gap < 0`, `REJECTED PRIMER`, etc.). Remplacement par des compteurs agrégés (`$skipped_inner_gap`, `$skipped_tm_diff`, `$chunk_reject`) gérés par le module `Parallel::ForkManager` et sommés via la méthode `run_on_finish`.
**Justification biologique** : Les boucles qui examinent toutes les combinaisons thermodynamiques (jusqu'à plusieurs millions) produisaient un volume de log excessif qui engorgeait la mémoire du système et la sortie standard (I/O bound). L'élimination des écritures répétées accélère le processus de découverte des amorces.
**Impact attendu** : Une réduction massive de la consommation mémoire (passage de plus de 600 Mo à quelques kilo-octets pour les logs de sortie standard). Temps d'exécution réduit (moins de latence I/O). Le fichier de log détaillé reste disponible via `--verbose_validation`.
